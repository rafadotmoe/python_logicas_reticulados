...
def p_expression_and(p):
    ...
    p[0] = ll.and_formula(p[1], p[3])

def p_expression_or(p):
    '''expression : expression OR expression'''
    p[0] = ll.or_formula(p[1], p[3])
...
def output_latex(proof, seq):
    ...
    proof_data = ll.lift_object_to_bussproofs(proof)
    formula_left = ll.lift_formula_to_latex_string(seq[0])
    formula_right = ll.lift_formula_to_latex_string(seq[1])
